...
function cargarCancionesFirestore() {
  db.collection("cancionesFavoritas")
    .orderBy("timestamp")
    .onSnapshot((snapshot) => {
...
if (agregarCancionForm) {
  ...
  agregarCancionForm.addEventListener("submit", function (e) {
    ...
    const match = url.match(
      /(?:https?:\/\/)?open\.spotify\.com\/(?:[a-zA-Z0-9-]+\/)?track\/([a-zA-Z0-9]+)(?:[/?].*)?/,
    );
    if (!match) {
...
function updateCounter() {
  ...
  function formatYearsMonthsDays(obj) {
    const years = obj.years || 0;
    const months = obj.months || 0;
  ...
  if (el) {
    el.textContent = formatYearsMonthsDays(d);
  ...
  if (slider) {
    ...
    prevBtn &&
      prevBtn.addEventListener("click", () => {
        ...
        setTimeout(() => slider.classList.remove("anim"), 420);
      });
    nextBtn &&
